...
function applyCursor(){
  ...
  // Браузер (замечено в Chromium) иногда не перерисовывает кастомный
  // курсор (cursor: url(...)) после того, как на элементе стояло
  // cursor: none, — сравнение со строкой, лежащей в style.cursor, тут не
  // спасает: она меняется исправно (marker → none → marker), а на экране
  // всё равно остаётся невидимый курсор ластика. Проявлялось на 2→3→2
  // (маркер → ластик → маркер): значок пропадал, 3→1 (перо) при этом
  // работал — оба факта совпадают с тем, что виноват именно переход из
  // 'none', а не совпадение соседних значений. Пустой сброс перед КАЖДОЙ
  // установкой форсирует честную перерисовку независимо от истории.
  stage.style.cursor='';
