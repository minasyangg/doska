...
/** Вход гостя. Единственные вызовы от имени anon: security-definer функции
    отдают доску, только если владелец включил гостевой доступ.

    Два пути. По токену — старый, выданные ссылки продолжают работать. По
    самому адресу доски — новый: раньше учитель включал гостевой доступ,
    отправлял обычную ссылку, и человека уводило на вход, потому что токен
    выпускался только кнопкой «Копировать». Настройка стояла, а не работала.
    Пропуском теперь служит сам адрес — он и так неугадываем, и именно его
    учитель и отправляет. */
async function guestRpc(fn, body) {
  const base = supabaseUrl();
  if (!base) throw new DbError('Supabase не настроен', { transport: true });
  let r;
  try {
...
const guestOpen = (boardId, guestToken) =>
  guestRpc('doska_guest_open', { p_board_id: boardId, p_token: guestToken });
const guestOpenByLink = boardId =>
  guestRpc('doska_guest_open_link', { p_board_id: boardId });
...
module.exports = {
  ...
  getGuestLink, setGuestLink, dropGuestLink, guestOpen, guestOpenByLink
};
