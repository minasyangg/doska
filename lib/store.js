...
function simplify(pts) {
  ...
  while (gaps.length) {
    const [a, b] = gaps.pop();
    const p0 = pts[a][2] ?? 0.5, p1 = pts[b][2] ?? 0.5;
    let far = -1, best = PRESS_EPS;
    for (let j = a + 1; j < b; j++) {
      const d = Math.abs((pts[j][2] ?? 0.5) - (p0 + (p1 - p0) * ((j - a) / (b - a))));
...
/** Копия объектов, пригодная для записи: линии прорежены, координаты до
    десятых, нажим до сотых. Десятая доля пикселя не видна на любом
    увеличении, а в JSON это три-четыре знака экономии на числе. */
function compact(items) {
  const out = new Array(items.length);
  for (let i = 0; i < items.length; i++) {
    const it = items[i];
    if (!Array.isArray(it.pts) || it.pts.length < 2) { out[i] = it; continue; }
    const pts = simplify(it.pts).map(p => [
      Math.round(p[0] * 10) / 10,
      Math.round(p[1] * 10) / 10,
      p.length > 2 ? Math.round(p[2] * 100) / 100 : undefined,
    ].slice(0, p.length));
    out[i] = { ...it, pts };
  }
  return out;
